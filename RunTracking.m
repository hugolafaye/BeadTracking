...
% they are set to default value if absent. Several ways to call the function:
%
% RunTracking(0,0)
...
%              structure (default 0).
%  boolGui   : boolean for using a user interface to set the tracking 
...
trackDataFilePrefix          = 'trackdata'; %prefix of the output file
...
boolComputeMotionStates      = false; %boolean for computing motion states
...
if trackParam.boolComputeMotionStates
    ...
    nbTraj=size(trackInfo,1);
    [trackData,trackNbStates]=ComputeMotionStates(trackData,trackParam,nbTraj);
    
    %save the tracking results updated with the motion states
    %and the parameters into a file
    fprintf('Saving results...\n');
    trackDataFile=[trackDataFile,'_motion_states'];
    save(fullfile(trackParam.pathResults,[trackDataFile,'.mat']),...
        'tsp','trackParam','trackData','trackInfo','trackNbStates');
    
    %export the variables to the matlab workspace if needed
    if boolAssign
        assignin('base','trackData',trackData);
        assignin('base','trackNbStates',trackNbStates);
    end
end
...
if trackParam.boolVisualizeTrajectories
    delete(gcp('nocreate')); %close parpool if openned
